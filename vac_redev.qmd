...
Beyond the statutory requirements, identifying vacant and potentially redevelopable parcels provides important context for the Housing Growth Plans by supporting suitability analyses in subsequent phases.
...
Vacant parcels represent land that is either currently undeveloped or previously developed but now considered vacant, and they may present direct development opportunities. While no statewide dataset of vacant parcels exists, the statewide CAMA and parcel dataset can be queried to identify parcels labeled as vacant.
...
![*Example map of vacant parcels identified*](maps/Vacant_Parcels.png){fig-alt="Example map of vacant parcels identified" width="350"}
:::
...
**Dataset:** [Connecticut Vacant Parcels (CT Housing Data Hub)](https://housingmaps.ct.gov/datasets/ctmaps::connecticut-vacant-parcels/about)

Full SQL queries used to generate this dataset are available in our [GitHub repository](https://github.com/ctgisoffice/housing/blob/main/SQL_Vacant_Land_Parcels.sql).
:::
...
The available data describing brownfield site locations is provided as an .xlsx file maintained by the Connecticut Department of Energy & Environmental Protection (CT DEEP).

**Dataset**: [Connecticut Brownfields Inventory (CT DEEP)](https://portal.ct.gov/deep/remediation--site-clean-up/brownfields/brownfields-site-inventory)
...
2.  Identifying brownfield parcels: Next, use the points to identify parcels that contain brownfield sites. This may be achieved with various queries, summary tools, or join tools that utilize spatial overlap. The following resources provide details and options for this process:
    ...
    - ArcGIS Pro: [Select by Location](https://doc.esri.com/en/arcgis-pro/latest/help/mapping/navigation/select-features-by-location.html), [Spatial Join](https://doc.esri.com/en/arcgis-pro/latest/tool-reference/analysis/spatial-join.html?tabs=dialog), [Summarize Within](https://doc.esri.com/en/arcgis-pro/latest/tool-reference/analysis/summarize-within.html?tabs=dialog)
    - QGIS: [Extract by Location](https://docs.qgis.org/3.44/en/docs/user_manual/processing_algs/qgis/vectorselection.html#extract-by-location) or [Select by Location](https://docs.qgis.org/3.44/en/docs/user_manual/processing_algs/qgis/vectorselection.html#select-by-location) within Vector Selection, [Join Attributes by Location](https://www.qgistutorials.com/en/docs/3/performing_spatial_joins.html)

::: {style="text-align:center;"}
![*Example map of brownfield site parcels identified*](maps/brownfields.png){fig-alt="Example map of brownfield site parcels identified" width="350"}
:::
...
::: {style="text-align:center;"}
![*Example map of LTI parcels identified using an applied threshold.*](maps/LTI_ratio.png){fig-alt="Example map of LTI parcels identified using the applied threshold." width="350"}
:::
...
::: {style="text-align:center;"}
![*Example map of large parcels identified.*](maps/Relative_Size.png){fig-alt="Example map of large parcels identified." width="350"}
:::
...
::: {style="text-align:center;"}
![*Example map identifying parcels based on low FAR values.*](maps/FAR.png){fig-alt="Example map identifying parcels based on low FAR values." width="350"}
:::
...
1.  **Construction and renovation year clustering analysis**
    ...
    **Why multivariate clustering?**
    ...
    Instead, it is recommended to use a multivariate clustering tool that relies only on the presence of shared traits. These tools identify optimized seed locations, or initial parcels, by maximizing their initial distance from each other, then repeatedly adjusting these seeds to minimize variability within clusters and increase variability between clusters. In this way, all parcels with available data are assigned to a cluster with shared features. Simplified, the process is:
...
3.  Interpreting results
    ...
    ::: {style="text-align:center;"}
    ![](images/mc_boxplot.png){fig-alt="multivariate clustering box plot example"}
    :::
...
![Multivariate Clustering results](maps/MultiVar.png){fig-alt="multivariate clustering box plot example" width="300"}

![Cluster 2 isolated](maps/MultiVar2.png){fig-alt="multivariate clustering box plot example" width="300"}
:::